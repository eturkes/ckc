//! Gate for task 0.13.1: one compact golden over the demo [`RunManifest`]
//! byte-locks every artifact a `ckc demo` run emits through its `content_hash`,
//! alongside round-trip determinism and JSON Schema stability for the type.
//! `run_demo` returns `run_pipeline`'s manifest unmodified, so this golden equals
//! exactly what 0.13.3 `run_replay` re-derives — the committed oracle a fresh
//! `ckc replay` run compares against. Cross-output-location determinism lives in
//! `demo.rs` (`run_demo_hashes_identically_across_tempdirs`); this gate adds the
//! value lock the demo gate leaves to a golden.
...
// ---------------------------------------------------------------------------
// Golden fixture: the real toy demo manifest, so the golden records the content
// hash of every emitted run artifact rather than a stand-in. The tempdir is
// bound to a local so it outlives the `run_demo` writes; only the in-memory
// manifest is returned. The fixture uses the pub `run_demo` because integration
// tests are a separate crate and cannot reach `pub(crate)` `run_pipeline`.
// ---------------------------------------------------------------------------

fn golden_run_manifest() -> RunManifest {
    let dir = tempfile::tempdir().unwrap();
    ckc_cli::pipeline::run_demo("research-kernel", false, dir.path()).unwrap()
}
...
fn regenerate() {
    ...
    write_type(&golden_run_manifest(), "run_manifest");
}
