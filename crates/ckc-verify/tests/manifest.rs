//! Gate for task 0.9.16: one compact golden over the verification manifest
//! byte-locks every `verify_all` artifact through its `content_hash`, alongside
//! round-trip determinism and JSON Schema stability for `VerificationManifest`.
//! Drift in any certificate, witness, the certificate graph, or the assurance
//! seed shifts that entry's hash and breaks `canonical`.
...
use ckc_verify::{VerificationManifest, verification_manifest};
...
// ---------------------------------------------------------------------------
// Assertion helpers (same contract as ckc-compile/tests/manifest.rs)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Golden fixture: the real toy verification manifest, so the golden records the
// content hash of every emitted verification artifact rather than a stand-in.
// ---------------------------------------------------------------------------

fn golden_verification_manifest() -> VerificationManifest {
    verification_manifest(&CompileBundle::load_toy())
}
...
golden_suite!(
    gs_verification_manifest,
    VerificationManifest,
    golden_verification_manifest,
    "verification_manifest"
);

// ---------------------------------------------------------------------------
// Integrity: every manifest entry names a real committed `certs/` artifact (task
// 0.9.15), so the compact hash-lock and the on-disk verification tree stay
// coupled — a path the golden happily records yet that no file backs is caught.
// ---------------------------------------------------------------------------

#[test]
fn manifest_paths_resolve_to_committed_files() {
    let manifest = golden_verification_manifest();
    assert_eq!(
        manifest.0.len(),
        24,
        "11 certificates + 11 witnesses + certificate graph + assurance seed"
    );
...
// ---------------------------------------------------------------------------
// Regeneration: `cargo test -p ckc-verify --test manifest -- --ignored`
// ---------------------------------------------------------------------------
...
fn regenerate() {
    ...
    write_type(&golden_verification_manifest(), "verification_manifest");
}
