//! Foundational value types of the domain model (SPEC §9).
//!
//! ```text
...
//! serialized as decimal strings; JSON numeric tokens are rejected (SPEC §10).
...
/// A stable lowercase-ASCII identifier matching `[a-z][a-z0-9_.:-]*` (SPEC §9).
...
/// A content hash, `"sha256:"` followed by 64 lowercase hex digits (SPEC §9).
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(try_from = "String", into = "String")]
...
/// with each integer encoded as a decimal string (SPEC §9, §10).
...
/// An exact reduced rational (SPEC §9), always stored gcd-normalized with a
...
mod tests {
    ...
    fn rational_serde_rejects_numeric_tokens() {
        // SPEC §10: JSON numeric tokens are rejected; the wire form is string-encoded.
