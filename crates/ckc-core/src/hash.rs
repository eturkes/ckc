//! Envelope content and policy hashes (SPEC §10).
//!
//! SPEC §10 fixes `content_hash = sha256(canonical_payload_bytes(payload))`.
...
//! for the §10 `_hash` fields that declare raw-byte hashing. The descriptor
...
/// SPEC §10 `content_hash = sha256(canonical_payload_bytes(payload))`, wrapped as
...
/// primitive behind [`content_hash`], and the entry point for §10 `_hash` fields
...
/// SPEC §10 `canonicalization_policy_hash`: the [`content_hash`] of the
...
/// Stable identity of the SPEC §10 canonical-JSON byte policy: an `id` naming the
...
/// [`content_hash`] over it; bumping `version` when §10 changes re-keys every
...
impl CanonicalizationPolicy {
    /// The M1 canonical-JSON byte policy (SPEC §10).
