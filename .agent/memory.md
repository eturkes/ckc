...
- Greenfield/speculative-milestone sizing (M3 APE-fork plan): MEASURED ANCHORS above are Rust read-cost-calibrated → they DON'T transfer to a no-existing-code milestone (nothing to byte-pin; external-discovery read replaces type-family read → no real anchor to SIZE-CHECK against). Plan-time moves instead: (1) hard-split every KNOWN multi-deliverable bundle (a line naming ≥2 artifacts, or a 'producer + its byte-pin battery' pair, splits BEFORE scheduling — e.g. drs-map.1→1a map/1b byte-emit, conformance-seed→a corpus/b runner); (2) PRE-DECLARE the respec seam IN each remaining atomic discovery-heavy line (`[half-A]` | `[half-B]`, split point = first-external-read boundary, e.g. first APE-DRS read) so an overrun recovers FORWARD without a cold respec — cheap insurance where M1/M2 reactive-only respec can't pre-size a greenfield unit.
