...
- Vendoring/fetch units: verify EACH source's OWN operative grant via its per-file source HEADER (APE `ape.pl` / AceRules `*_processor.pl` first ~25 L = the LGPL-3.0-or-later grant; do NOT assume that grant extends to ape-build's Clex/acetexts — each carries + needs its own). Add a `LICENSE.txt` TITLE spot-check (first ~15 L). Bank in the unit spec so execution needs ~zero rediscovery: exact pins + `HEAD^{tree}` hashes (confirm-not-rediscover), narrow-read ranges, placement via `git archive HEAD <paths>|tar -x`. Whole-file `LICENSE.txt` / big-source reads are the rediscovery cost these units must avoid — narrow-read the header, never the license body. EXEC-TIME COROLLARY (ape-vendor 2026-07-14): a FAST-PATH's "probed + first-hand-checked" banked facts STILL need first-hand re-probe at fetch — ape-vendor's carried 2 falsified (owlswrl at `prolog/utils/owlswrl/` NOT `prolog/owlswrl/`; APE © = Attempto/UZH + Kuhn + Kaljurand *co-held* per per-file headers + manifest-wide sweep, not "owlswrl-only-Kaljurand"), caught + corrected into SPEC §11.5 + `CKC_FORK.md`. The placed APE `.gitignore` (`clinicalcnl/.gitignore`: `/ape.exe` `/ape-*.zip` `/packages` `*~`) already ignores ape-build's `ape.exe` output → no `git add -f`, never commit the binary.
