...
- "Oracle" carries two senses; the `terms:` cleanup (definitive map, since removed) renamed only the epistemic-overclaim one: `runtime-oracle`/`G-RUNTIME-ORACLE`/`RuntimeOracleReport`/"oracle fidelity" → `runtime reference`/`G-RUNTIME-REFERENCE`/`RuntimeReferenceReport`/"reference-fidelity" (results are locked measurements, not an authority on real-world truth), applied in SPEC.md + code with `docs/` out of scope. The TEST-ORACLE sense — the authority deciding a test's pass/fail against the reference — is accepted project vocabulary, kept in `run_oracle.rs` (file + `run_oracle_*` fn), `rules.rs` (`// THE oracle`), the `cli-runner.2c` roadmap stub, and docs themselves ("test oracle"/"SAT oracle"/"perfect oracle"); it survived adversarial review untouched. Leave it on any "oracle" sweep — retiring the test-sense globally is a separate, broader call.
