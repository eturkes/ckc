...
- [ ] run-m2.1e-C1: report ModelRunSections wiring + report_ja.md landing (behavior; crate green on existing tests — report_ja.md forces a one-line insert into 2 M1 + 1 M2 exhaustive file-listing pins — new VALUES unasserted, the B2a compute-then-green half). GOAL: `report_processing_stage` CONSTRUCTS `ModelRunSections` on a model route (`None` on M1) and hands it to `assemble_report` (currently hardcoded `None`), and lands report_ja.md beside report_en.md in the SHARED stage (BOTH M1 and M2 paths). `assemble_report`'s `Some`-arm (report.rs:785, report-m2.2 landed) ALREADY populates failure_taxonomy/metrics/model_identity — do NOT touch report.rs. Every shape below is source-read + banked → implement WITHOUT re-reading report.rs/metrics.rs/registry.rs. `ModelRunSections<'a>` (report.rs:551) = `{route_diagnostics:&'a[(Id,&'a[DiagnosticRecord])], route_metrics:Vec<RouteMetrics>, baseline_pipeline_id:&'a Id, model_identity:&'a ModelIdentity}`; `assemble_report` 8th param = `model_run:Option<ModelRunSections>` (report.rs:601). `RouteRun` (struct l.289) = `{pipeline_id:Id, ledger:Vec<DiagnosticRecord>, fills:Vec<FillObservation>, groups:Vec<GroupObservation>, samples:Vec<Vec<GroupObservation>>}`. `route_metrics(pipeline_id:&Id, fills, groups, samples, reference:&[ReferenceEntry])->RouteMetrics` (metrics.rs:526). `parse_reference(yaml:&str)->Result<Vec<ReferenceEntry>,_>` (ckc-core registry.rs; ReferenceEntry NOT named — inference types `reference`). EDITS (all run.rs): (1) IMPORTS — l.68 `use crate::report::{Report, assemble_report, render_markdown}` append `render_markdown_ja, ModelRunSections`; l.62 `use crate::metrics::{FillObservation, GroupObservation}` append `RouteMetrics, route_metrics`; add `parse_reference` to the `ckc_core::{…}` block (l.47-54). (2) `report_processing_stage` (fn l.2052) — APPEND `route_runs: &[RouteRun]` as the LAST param. (3) NEW helper near it: `fn model_route_metrics(root:&Path, resolved:&Resolved, route_runs:&[RouteRun], model_routes:&[RouteShape]) -> Result<Option<Vec<RouteMetrics>>, String>` — `if model_routes.is_empty() {return Ok(None)}` else: `let experiments = parse_experiments(&std::fs::read_to_string(root.join(EXPERIMENTS_FILE)).map_err(|e| e.to_string())?).map_err(|e| e.to_string())?;` (`parse_experiments`/`EXPERIMENTS_FILE` imported/used at l.2295); `let exp = experiments.into_iter().find(|e| e.id==resolved.plan.experiment_id).ok_or_else(|| "report metrics: experiment absent".to_owned())?;`; `let reference = parse_reference(&std::fs::read_to_string(root.join(&exp.expected_outcomes)).map_err(|e| e.to_string())?).map_err(|e| e.to_string())?;`; `Ok(Some(route_runs.iter().map(|rr| route_metrics(&rr.pipeline_id,&rr.fills,&rr.groups,&rr.samples,&reference)).collect()))` (experiment lookup mirrors manifest_inputs l.2293-2300; reference-parse mirrors run_oracle.rs:190-192 — manifest_inputs raw-reads bytes but route_metrics needs a parsed `Vec<ReferenceEntry>`; model-route-only, so M1 never runs the duplicate parse). (4) BODY — before the `let landed =` chain build `let route_diagnostics: Vec<(Id,&[DiagnosticRecord])> = route_runs.iter().map(|rr| (rr.pipeline_id.clone(), rr.ledger.as_slice())).collect();` (empty on M1); then CHANGE the chain's first call (l.2082-2093, `assemble_report(…, None)`) to `model_route_metrics(root, resolved, route_runs, model_routes).map_err(|reason| fail(format!("report model sections: {reason}"))).and_then(|route_metrics| { let model_run = match (route_metrics, agreed.as_ref()) { (Some(metrics), Some(model_identity)) => Some(ModelRunSections { route_diagnostics: &route_diagnostics, route_metrics: metrics, baseline_pipeline_id: &resolved.pipeline_id, model_identity }), _ => None }; assemble_report(&bundle.payload, &lineage.payload, &graphs.iter().map(|g| &g.payload).collect::<Vec<_>>(), &results.iter().map(|r| &r.payload).collect::<Vec<_>>(), lexicon_hash, solver_identity, &ledger, model_run).map_err(|e| fail(format!("report assembly: {e}"))) })` — the match MOVES `metrics` in (no clone) and borrows `model_identity` via `agreed.as_ref()`; `agreed=None` (no route attested an identity — `agree_model_identity` treats absent as vacuous agreement, no fail-close, so a degraded model run reaches here with None) degrades to `model_run=None` (mirrors the manifest's graceful omit; `ModelRunSections` requires a `&ModelIdentity`, so model sections are all-or-nothing — NOT an `.expect()` panic); `agreed` is the `Option<ModelIdentity>` param (by value, borrow lives the fn); rest of the chain (validate+wrapper+land+report_en+manifests) stays verbatim. (5) report_ja BLOCK — after the report_en.md `.and_then` block (ends ~l.2128 `Ok(report) })`) INSERT a byte-for-byte copy with en→ja: `.and_then(|report| { let body = render_markdown_ja(&report.payload); let path = shell.write_under("report_ja.md", body.as_bytes()).map_err(|e| fail(format!("report_ja.md: {e}")))?; let read_back = std::fs::read(&path).map_err(|e| fail(format!("report_ja.md: read back: {e}")))?; if read_back != body.as_bytes() { return Err(fail("report_ja.md: read back diverges from the rendering".to_owned())); } Ok(report) })`. (6) CALL SITES — M1 (l.267, ends `…, true, None, &[]`) append `, &[]`; M2 (l.640, ends `…, false, agreed, &model_routes`) append `, &route_runs` AND delete `let _ = &route_runs;` (l.611). (7) LISTING PINS — report_ja.md lands via `shell.write_under` (a plain write, NOT `land`/`wrapper`/`land_record`) and the manifest derives from the report wrapper (`manifest_inputs(…, &report, …)`), not a dir scan, so every BYTE-pinned M1 record (report.json, report_en.md, manifest.json, replay_manifest.json, lineage_index.json) stays byte-identical — the added file perturbs ONLY exhaustive file-SET listings. THREE exist, all get a one-line insert: (i) run.rs M2 root-listing `assert_eq!` (l.3923-3934 in `m2_route_loop_lands_both_routes_namespaced`, `"report_en.md"` at l.3930) insert `"report_ja.md",` after it; (ii) tests/cli_shell.rs M1-run `sorted_entries(&out_dir)` literal (`"report_en.md",` at l.113) insert `"report_ja.md",` after it; (iii) tests/run_oracle.rs M1 `expected_files` root vec (`"report_en.md".into(),` at l.206) insert `"report_ja.md".into(),` after it (vec sorted at l.500 before the exhaustive `files_under` walk-compare l.501). Unaffected (verified, no edit): per-route + gate-failure listings (`["logs"]` l.2855 / `["logs","routes"]` l.4659/5050/6019) never reach the report stage; the determinism walk (l.4229) compares two runs to each other; all other report_en.md sites are targeted `.exists()`/content reads (run.rs:3038/3464, run_oracle.rs:789, replay/command.rs:124). Gate: fmt/clippy/`RUSTDOCFLAGS='-D warnings' cargo doc -p ckc-cli --no-deps`/`cargo test -p ckc-cli`. Acceptance: sections flow into assemble_report on model routes (None on M1, and on a model route that attested no identity → model_run None, no panic); report_ja.md lands both paths; the three exhaustive file-set listings (1 M2 + 2 M1) updated so the crate is green; M1 report.json/report_en.md/manifest.json/replay_manifest.json/lineage_index.json byte-identical. Read ONLY: run.rs report_processing_stage body (2052-2160) + both call sites + RouteRun struct — all banked here; the 3 listing-pin inserts (run.rs:3930, tests/cli_shell.rs:113, tests/run_oracle.rs:206) are single-line literal additions banked in (7); SOURCES (report.rs/metrics.rs/registry.rs) EXCLUDED.
- [ ] run-m2.1e-C2: 3-body observed-output pin battery over the model-route run (tests-only, bless-from-observed; the B2b value-pin half — C1 landed the behavior, its existing-test pass leaves these VALUES unasserted). Extend `m2_route_loop_lands_both_routes_namespaced` (run.rs ~3902, over `write_m2_root`) with: (a) report.json M2 sections — `strict_at::<crate::report::Report>(&out, "report.json")` (pattern l.3464); assert `failure_taxonomy` (per-route code→count map from each route's §7.4 ledger), `metrics` (all raw_rows strictly before route_deltas per ExperimentMetrics::emission_order; `k_sample_convergence` = `not_applicable`, single k=1 draw), `model_identity` == the golden cassettes' synthetic identity `model.baseline`/`fixture_quant`/`1.0.0` (route.single_ir + route.direct_smt goldens agree = the run's agreed identity); (b) report_en.md full body — read `out.join("report_en.md")`, `assert_eq` vs a blessed const (M2 now renders: failure-taxonomy section after diagnostics summary, metrics section via emission_order, model-identity section after solver identity); (c) report_ja.md full body — same, blessed const (identical section walk under JA_LABELS; §0 vocab + ids/hashes/codes + `not_applicable` verbatim EN, `なし。` empty slot). BLESS from observed, never hand-compute: capture the landed report_en.md/report_ja.md/report.json from a write_m2_root run, embed as consts, re-run green. Full-body pin shape mirrors report.rs:3552 `render_markdown_ja_pins_the_populated_m2_view` (~130-line const) + :3697. Gate: fmt/clippy/doc/cargo test. Acceptance: the three M2-section value sets pinned from observed bytes, guarding regressions C1's existing-test pass cannot catch. Read: run.rs write_m2_root (3860) + m2_route_loop test body (3902-4110) + `strict_at` helper; report.rs render_markdown/_ja pin-test shape (3552/3697). C1's wiring is landed — do NOT re-touch it.
