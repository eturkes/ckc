...
- [ ] model-adapter.2: constrained generation + k-sample (live). Complete `invoke` for real
  ...
  collect k outputs + recorded-call count. HARDEN capture byte-completeness (deferred from .1
  codex-review): a descendant outliving a clean exit while holding the stdout pipe truncates
  `Completed`/`stdout_bytes` past DRAIN_GRACE → kill the process group or gate `Completed` on stdout
  EOF (a capture-incomplete outcome); the generated bytes are byte-stability-load-bearing here.
  Reading: model-adapter.1 module; `schemas/` outputs; the
