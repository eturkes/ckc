...
- [ ] model-adapter.2a-codexfix: apply round-2 `/codex-review` accepted findings to the committed .2a surface (9ae5773 — post-commit review; the round-1 refinements noted in .2a's [Done] were incomplete, this closes the gap). Pre-decided, near-mechanical.
  FAST PATH → `git apply .agent/codex-fixup-2a.patch` (untracked, validated APPLY_OK; carries model.rs + schemas/smt_query.grammar + memory.md = findings #1-5,7,8a,9 verbatim) THEN the hash re-pin below; or re-derive from this ledger if the patch is gone.
  #1 High — `Completed` snapshot race: load `stdout_eof` BEFORE snapshotting `stdout_buf` (the drain thread sets eof only after appending its final chunk → eof=true ⟹ the following snapshot is whole; eof=false ⟹ `CaptureIncomplete`, conservatively safe). #2/#3/#4 doc honesty (model.rs `ModelRun`/`DRAIN_GRACE`): Timeout = TYPICALLY a prefix, not guaranteed (a descendant outliving the budget-kill can add bytes past it); ExitFailure is NOT EOF-gated → completeness UNPROVEN (usually whole, but short on a slow/large finite drain or a descendant holding the pipe); residual = a detached drain thread (+ a stdin writer blocked on a large unread prompt) can outlive the call — the `forbid(unsafe)` no-process-group-kill stance accepts it. #5 `derive_seed` doc: decorrelates sequential i (adjacent samples seed the runtime PRNG from unrelated values); whether draws diversify is the runtime's decode mode, out of scope (→ `invoke_samples`). #7 test `probe_holding_stdout_open_fails_construction`: stub echoes identity then backgrounds `sleep` holding stdout → `with_command` returns `Err(ModelAdapterError::Probe{detail})`, `detail.contains("never reached EOF")`. #9 memory:131 stale (claims 2 sentinels / completeness deferred / enum spec-locked to 4 variants) → .2a actually has 3 sentinels + `CaptureIncomplete` (EOF-gated); patch corrects it.
  #8 engine-name DE-LEAK (hard invariant — committed SPEC/code/registry/roadmap/schemas name no inference engine, grammar dialect, or model-file format; round-3 `/codex-review` widened the scope past round-2's lone grammar:5 + roadmap:98): (8a) `schemas/smt_query.grammar:5` drop "EBNF/GBNF" (in patch). (8b) roadmap dialect names — NOT in patch, pure text, land in the schema commit: schemas-export.2 stub `the canonical base of EBNF/GBNF` → `the canonical base`, `rejected pest/PEG = tool-coupled` → `rejected tool-coupled alternatives`, line ~110 `no EBNF` → `no extended-grammar`. (8c) `.agent/memory.md` "Live smt_query grammar hash" line — drop the duplicated `f14a266…`/`2492 B` (the GRAMMAR_HASH const in emit.rs + the schemas.yaml pin are canonical; a third copy drifts, and 8a makes it 2489 B). (8d) on close, scrub this unit's own collapsed [x] stub of every EBNF/GBNF/pest/PEG token. KEEP as neutral self-description (codex-confirmed clean, and editing them would re-shift the hash): "Backus-Naur"/"BNF" + "ABNF-style `;` comments" at grammar:2 + memory:291. HASH RE-PIN — the 8a edit shifts the grammar sha256: replace OLD `f14a26688f7540f745d2e00a2b4b3e3a8627ee6ee8819b90d0c04547a011ab6e` → NEW `d26bbd5b9067a597acdb10855099707677cbfb363229f2bc13242637de0f97f2` (precomputed for this exact patch; CONFIRM `sha256sum schemas/smt_query.grammar` post-apply) at the only 2 sites — `registry/schemas.yaml:7` + `crates/ckc-smt/src/emit.rs:1025`. (Grammar RULES unchanged → recognition identical; the 2 `registry_check` tests stay red until both re-pinned.) VERIFY de-leak: `git grep -niE '(ebnf|gbnf|pest|peg)' -- ':!docs/'` → 0 hits (docs/ = out-of-scope reference corpus).
  REJECTED #6 — `derive_seed` stays private: Z3Adapter-mirrored, seed already exposed via `ModelSample.seed`, replay re-derives through `invoke_samples`; no consumer needs it public.
  GOTCHA: re-deriving (not patch-applying) → anchor each Edit `old_string` via `command grep -n` — Headroom Read re-wraps prose, so a pasted anchor won't match.
  Gate: `cargo test --workspace` (expect 410 passed / 1 ignored — +1 for #7) + `cargo fmt --all --check` + `cargo clippy --workspace --all-targets -- -D warnings` green.
  CLOSE: two scoped commits, each + `Codex-Review:` trailer, local only (I handle remote) — (1) `model (M2.9): codex round-2 — Completed race fix + capture/seed doc honesty + probe test` (model.rs + memory.md:131; trailer #1,2,3,4,5,7,9); (2) `schemas (M2.9): de-leak grammar+roadmap dialect names + re-pin oracle/registry` (grammar + registry/schemas.yaml + emit.rs + roadmap 98/99/110 + this unit's closing stub + memory.md "Live…hash" line; trailer #8 + round-3 de-leak-scope + memory-staleness findings), which also marks this unit DONE + `rm .agent/codex-fixup-2a.patch`. Context-tight → land (1), defer #8 to a fresh session (clean split; #8 stays small — pure-text scrubs + 2 hash re-pins + a verify grep). Record context-usage.
