...
- [ ] model-adapter.2a-codexfix: apply round-2 `/codex-review` accepted findings to the committed .2a surface (9ae5773 — post-commit review; the round-1 refinements noted in .2a's [Done] were incomplete, this closes the gap). Pre-decided, near-mechanical.
  ...
  #8 engine-name DE-LEAK (hard invariant — committed SPEC/code/registry/roadmap/schemas name no engine/dialect/format): (8a) `schemas/smt_query.grammar:5` drop "EBNF/GBNF" (in patch); (8b) roadmap schemas-export.2 stub `the canonical base of EBNF/GBNF →` → `the canonical base →` (one-liner, NOT in patch). HASH RE-PIN — the 8a edit shifts the grammar sha256: replace OLD `f14a26688f7540f745d2e00a2b4b3e3a8627ee6ee8819b90d0c04547a011ab6e` → NEW `d26bbd5b9067a597acdb10855099707677cbfb363229f2bc13242637de0f97f2` (precomputed for this exact patch; CONFIRM `sha256sum schemas/smt_query.grammar` post-apply) at the only 2 sites — `registry/schemas.yaml:7` + `crates/ckc-smt/src/emit.rs:1025`. (Grammar RULES unchanged → recognition identical; the 2 `registry_check` tests stay red until both re-pinned.)
  ...
  CLOSE: two scoped commits, each + `Codex-Review:` trailer, local only (I handle remote) — (1) `model (M2.9): codex round-2 — Completed race fix + capture/seed doc honesty + probe test` (model.rs + memory.md; trailer #1,2,3,4,5,7,9); (2) `schemas (M2.9): de-leak grammar dialect name + re-pin oracle/registry` (grammar + registry/schemas.yaml + emit.rs + roadmap:98; trailer #8), which also marks this unit DONE + `rm .agent/codex-fixup-2a.patch`. Context-tight → land (1), defer #8 to a fresh session (clean split). Record context-usage.
