...
- Module home = ckc-cli fresh modules (`cnl.rs`, `cnl_grammar.rs`, `cnl_parse.rs`,
  ...
  impls outside core proven (report.rs), consumers all CLI-side. ckc-core + committed
  clinical_ir.schema.json UNTOUCHED (no IR field change — ClinicalStatement already carries
  certainty/exceptions/source refs).
- CNL AST ≠ ClinicalIr: CnlAtom {Concept|ConceptNegated|Interval|Unregistered(surface)}
  (escape = own variant), CnlContext {any: Vec<Vec<CnlAtom>>} flat two-level DNF,
  CnlException (DNF), CnlRule {context, action kind+target, direction+strength, certainty?,
  exceptions, basis region refs}, CnlDocument {document_id, rules} = the landed model_fill
  artifact payload (AST only — canonical TEXT is derived by render; text hashes ride
  report.json). Parser mints NO ids; bridge derives them.
- Bridge determinism: one ClinicalStatement per context-disjunct (ids `<doc>.rule.<k>` in
  ...
  concept (system = lexicon.system, code = concept id); basis refs = region ids;
  source_segment_ids derived region→segment via the graph.
- Grammar terminals = whole-surface string literals ONLY (no char classes/ranges; ASCII digits
  as literal alternation) — portable to LLM constraint mechanisms + atomic in bnf. bnf 0.6
...
- [ ] lexicon-cnl.1: CNL surface fields — LexiconConcept +adnominal_ja/negated_ja/gloss_en,
  ...
  strict extension (deny_unknown_fields holds, surfaces through StringPolicy::SemanticJa
  normalization like existing); ja_core.yaml authored for the full M1 set (6 concepts,
...
- [ ] cnl-ast: cnl.rs type family — CnlAtom/CnlContext/CnlException/CnlRule/CnlDocument +
...
- [ ] cnl-grammar.1: cnl_grammar.rs emitter — clinical_cnl_grammar(lexicon, lang) -> Vec<u8>
  ...
  concept|negated|interval|escape (未登録概念「…」 / unregistered concept "…"); EN mirror
  productions; terminals = lexicon whole-surface literals; interval numerals = ASCII-digit
  literal alternation. Oracle tests in-crate (bnf workspace dev-dep added to ckc-cli): §10
...
- [ ] cnl-grammar.2: committed schemas/clinical_cnl_{ja,en}.grammar (ignored bless +
  never-writes drift guard + hash-pin consts — schema.rs pattern) + schemas.yaml entries
  schema.clinical_cnl_{ja,en} (target_kind cnl_document) + .gitattributes eol=lf + registry
  check green + committed_model_surface_checks_ok drift-guard extension.
...
- [ ] cnl-render: cnl_render.rs — render_ja/render_en canonical text (modality pair → the
  ...
  control shape).
- [ ] cnl-bridge: cnl_bridge.rs — to_ir per the plan-header determinism rules (region→segment
  derivation via graph regions — read ClinicalSegment's shape at impl) + from_ir projection
  (one CNL rule per statement) + to_ir∘from_ir == id property + worked-example content test
...
- [ ] cnl-laws: depth-bounded AST enumeration harness (all atom kinds × ≤2 disjuncts × ≤2
  ...
  single-parse (take(2) Earley differential over a bounded sample) + to_ir/from_ir round-trip.
  Codeco method; bound sizes to CI-sane runtime.
...
- [ ] route-single-cnl.1: registry data — processing_stage.m3.model_fill_cnl (model_fill,
  [sdg,segments]→[cnl_document]) + m3.bridge (bridge, [cnl_document]→[clinical_ir]) +
  pipe.m3_single_cnl (7 stages; m2.assemble + m1.compile/m1.verify reused) + exp.m3_cnl set
  ...
  select_record_{schema,prompt} keys (JA grammar = the decoding constraint) +
  resolve-rejection additions + registry check green. run.rs read scope: resolve/consts +
  prompt-composer regions only.
- [ ] route-single-cnl.2: single_cnl_accept closure — parse (Parse reject, repairable) →
  escape scan (Unregistered terminal) → grounding (basis regions ⊆ regions, derived segments
...
- [ ] route-single-cnl.3: single_cnl_fill + execute_routes SingleCnl arm (head reuse → fill →
  bridge + assemble tail into the existing per-group compile/verify loop) + landing
...
- [ ] route-single-cnl.4: golden CNL cassettes ×3 (hand-authored canonical JA CNL bodies for
  ...
  run_oracle mirror) + event/landing pin battery (census, layout, event input_hashes pinned
  independently — the .1d4b lesson).
- [ ] route-single-cnl.5: rejection cassettes (parse-error → derived-seed repair → recover /
...
- [ ] report-cnl.1: Report shape — cnl_documents (per-doc {ja,en} text hashes) + cnl_rules
  ...
  half).
- [ ] report-cnl.2: population + audit views — assemble_report CNL inputs (from_ir + render
  over every accepted ClinicalIr, any route incl. M1) + run.rs report-tail lands
...
- [ ] record-cnl.1 (gated: model runtime): constraint/tokenizer audit — env wrapper compiles
  ...
  multibyte whole-surface terminals survive the runtime constraint mechanism (§9 truncation +
