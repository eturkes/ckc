...
- [ ] run-m2.1e-B2a: §9 COMPUTED on model routes — computation+threading+fixture, leaves crate GREEN on existing tests (VALUES unpinned → B2b). B split recurses (memory sizing): B2's comp+threading+value-pins overflowed, so split once more at the same seam. GOAL: `manifest_inputs` (run.rs:2171) populates the 5 hashable §9 fields on model-route runs, 7×None on M1; `model_hash`/`runtime_hash` STAY None (SPEC §794-795/854 — model+runtime = env bare-name PATH commands, no committed bytes; identity rides `model_identity`). GATE ON MODE not presence (codex Finding 2: `agreed=None` on all-fills-failed ≡ M1 all-None → aliases two modes): new LAST params `agreed: Option<&ModelIdentity>, model_routes: &[RouteShape]`; body `if model_routes.is_empty() { (None ×7 — M1 path, byte-identical) } else { model_identity=agreed.cloned() (HONEST None when no identity observed — an error run records input hashes + None identity, distinct from M1 all-None); compute the 4 hashes }`. §9 covers THIS run's inputs, NOT registry-wide (codex Finding 1). Helper before `manifest_inputs`: `fn aggregate_hashes(mut v: Vec<String>) -> Hash { v.sort(); v.dedup(); hash_bytes(v.join("\n").as_bytes()) }`. The 4 (COMPILE-SHAPE: each hash field is `Option<Hash>`→wrap `Some(...)`; the fn returns `Result<_,String>` so a bare `?` on io::Error/RegistryError will NOT convert → funnel every new `std::fs::read`/`read_to_string` + `parse_*` `.map_err(|e| format!("manifests: <what>: {e}"))?` like the existing `read {LOCKFILE}` :2191, and the experiment `.find` `.ok_or_else(|| "manifests: experiment absent".to_owned())?`): `test_source_hash`=`Some(aggregate_hashes(docs.iter().map(|d| d.source_hash.as_str().to_owned()).collect()))` (docs route-expanded; sort+dedup collapses per-route dupes); `reference_hash`=`Some(hash_bytes(&std::fs::read(root.join(&exp.expected_outcomes)).map_err(..)?))`, `exp`=`parse_experiments(&std::fs::read_to_string(root.join(EXPERIMENTS_FILE)).map_err(..)?).map_err(..)?.into_iter().find(|e| e.id==resolved.plan.experiment_id).ok_or_else(..)?` (experiment_id already used ~:2187); `schema_hash`/`prompt_template_hash` ROUTE-RELEVANT — per `model_routes` entry fill `BTreeSet<Id>` want_schema/want_prompt via static_id (SingleIr→(schema.clinical_ir,prompt.single_ir), DirectSmt→(schema.smt_query,prompt.direct_smt), M1Layered→continue; grounded: run.rs forms schema.<output_kind>, prompts.yaml 1:1), then `schema_hash`=`Some(aggregate_hashes(parse_schemas(&std::fs::read_to_string(root.join(SCHEMAS_FILE)).map_err(..)?).map_err(..)?.iter().filter(|s| want_schema.contains(&s.id)).map(|s| s.schema_hash.as_str().to_owned()).collect()))`, `prompt_template_hash` same over `parse_prompts(&std::fs::read_to_string(root.join(PROMPTS_FILE)))` filtering `want_prompt.contains(&p.id)` mapping `|p| p.template_hash.as_str().to_owned()`. CONSTS (add near CORPORA_FILE run.rs:85; dedup existing string literals at resolve's `load` :698 + copy_committed_registry): EXPERIMENTS_FILE="registry/experiments.yaml", SCHEMAS_FILE="registry/schemas.yaml", PROMPTS_FILE="registry/prompts.yaml". IMPORT (ckc_core use-block run.rs:53 — parse_experiments/read_strict_canonical/ModelIdentity ALREADY present): add parse_prompts, parse_schemas. THREADING: report_processing_stage (run.rs:2032) gains the two params, forwards `agreed.as_ref(), model_routes` to the manifest_inputs call (:2107); execute M1 tail (report_processing_stage call :255) passes `None, &[]`; execute_routes (:329) declares `let mut model_routes: Vec<RouteShape>` and pushes `resolved.shape` beside route_runs.push (:583, `resolved` = loop var), M2 tail (:622) passes `agreed, &model_routes` (agreed = `let mut agreed: Option<ModelIdentity>` :371, last use). FIXTURE FIX (REQUIRED — manifest_inputs now reads schemas/prompts, so model-route tests must provide them; the un-banked version of this gap = the debug loop that overflowed B2): copy_committed_registry (run.rs:3211) copies [CORPORA_FILE, "registry/candidates.yaml", "registry/experiments.yaml", TOOLCHAIN_FILE] → add SCHEMAS_FILE, PROMPTS_FILE; its 3 non-model callers (resolve-battery: missing_model_repair_limit_stops_resolution / model_repair_limit_beyond_u32_stops_resolution / model_sample_count_beyond_one_stops_resolution) never reach manifest_inputs → harmless. (reference file corpus/reference/m1_expected.yaml already copied — scoring reads it — so only schemas/prompts are missing.) DOC-ALIGN (the one source edit beyond run.rs — doc-only, no behavior/bytes): plans.rs RunManifest §9 docs :185-198 drifted from the route-relevant + None-in-replay reality that manifests.rs `ManifestInputs` §9 docs already state → retune schema_hash/prompt_template_hash off the registry-wide 'committed schemas/ export / prompt-template registry in force' to ROUTE-relevant wording, and add the 'None while an env command' caveat to model_hash/runtime_hash (manifests.rs already carries both — no edit there). ACCESSORS (BANKED — do NOT re-read source modules): ModelIdentity{model_id:Id, quant:String, runtime_version:String} (Clone/PartialEq/Eq, ckc-core, imported); RouteShape::{SingleIr,DirectSmt,M1Layered} (Copy, in-module); SchemaEntry{id:Id, schema_hash:Hash}, PromptEntry{id:Id, template_hash:Hash}, ExperimentEntry.expected_outcomes:String (ckc-core registry.rs — inferred via parse_* returns, no type import); parse_{schemas,prompts,experiments}(&str)->Result<Vec<_>,RegistryError>; hash_bytes(&[u8])->Hash; Hash::as_str()->&str; DocTrace.source_hash:Hash; static_id(&str)->Id; resolved.{shape, plan.experiment_id}. READ ONLY (minimal apply-anchor set; SOURCES excluded): run.rs use-block :49-54, consts :77-90, execute M1 tail :255, execute_routes :371-630 (agreed decl + route loop + RouteRun push + M2 tail), resolve `load` :698, report_processing_stage sig+call :2032-2130, manifest_inputs sig+body+struct-literal :2171-2230, copy_committed_registry :3211-3224, plans.rs:185-198 (RunManifest §9 docs — DOC-ALIGN edit only). GATE: fmt/clippy/`RUSTDOCFLAGS='-D warnings' cargo doc -p ckc-cli --no-deps` (17 pre-existing private-intra-doc-link errors = standing baseline — diff the LIST vs model.rs(13)/replay.rs(2)/trace.rs(2), not the count)/`cargo test -p ckc-cli` ALL GREEN (existing model-route tests run the §9 path → prove it computes without erroring; §9 VALUES unasserted here); engine-agnostic word-boundary grep over run.rs + plans.rs (touched files). Acceptance: §9 computed route-relevantly on model routes (model_hash/runtime_hash None), M1 7×None, every existing test green (M1 manifest pins byte-identical via omit-None); plans.rs §9 record docs aligned (route-relevant + None-in-replay).
- [ ] run-m2.1e-B2b: §9 VALUE PINS over the replayed model-route run (tests-only, bless from observed). Depends on B2a (computation landed). Extend `m2_route_loop_lands_both_routes_namespaced` (run.rs:3755 — its inner block ~:3894 already does `use ckc_core::RunManifest; let manifest: RunManifest = read_strict_canonical(&std::fs::read(out.join("manifest.json")).unwrap()).unwrap();` for the .1e-A output_hashes.contains pins): add model-route §9 asserts on that same `manifest` — `model_identity`==Some(the run's SYNTHETIC identity ModelIdentity{model_id: static_id("model.baseline"), quant: "fixture_quant".to_string(), runtime_version: "1.0.0".to_string()}), `model_hash`==None, `runtime_hash`==None, the 4 hashes each Some; PIN VALUES via `Hash::as_str()` projections — `reference_hash` (=hash_bytes over corpus/reference/m1_expected.yaml = exp.m2_multihop's expected_outcomes, fixture-provided), `schema_hash` (aggregate over {schema.clinical_ir, schema.smt_query}), `prompt_template_hash` (over {prompt.single_ir, prompt.direct_smt}), `test_source_hash`. BLESS FROM OBSERVED, never hand-compute (memory): write the asserts with placeholder literals, run once, read the real `sha256:…` values from the assert-failure diff, then pin. ReplayManifest parity: `use ckc_core::ReplayManifest;` read replay_manifest.json the same way, assert its 7 §9 fields == the RunManifest's (7-tuple equality — covers both records without recomputing). M1: existing M1 manifest byte-pins already guard §9-all-None byte-identity (omit-None) — add an explicit `model_hash==None`/etc. on one M1 manifest test only if trivially cheap. Determinism (run-m2.1d5b two-run byte-equality) + partial-landing (m2_direct_partial_landing contains-pins) tests are §9-invariant → already pass, no new pins. READ ONLY: run.rs m2_route_loop test region :3755-3960 (its manifest.json read pattern) — do NOT re-read manifests.rs/plans.rs (RunManifest/ReplayManifest §9 field names BANKED, plans.rs:185-198 RunManifest + :323-329 ReplayManifest, identical set: model_identity, test_source_hash, reference_hash, schema_hash, prompt_template_hash, model_hash, runtime_hash — all Option). GATE: fmt/clippy/`RUSTDOCFLAGS='-D warnings' cargo doc -p ckc-cli --no-deps` (diff LIST not count)/`cargo test -p ckc-cli`; engine-agnostic word-boundary grep over run.rs. Acceptance: model-route §9 values pinned in both records (RunManifest + ReplayManifest parity), M1 §9-None held.
- [ ] run-m2.1e-C: report ModelRunSections + report_ja.md. SIZE-CHECK at start vs the B2 recursion rule (memory sizing): B2a discharges the threading, so C = sections-build + report_ja land + a 3-body byte-pin battery (report.json M2 sections / report_en.md / report_ja.md) over report.rs+metrics.rs reads — if those miss one-window co-fit, split at that seam (report_ja + the pin battery as the second half). `report_processing_stage` builds `ModelRunSections{route_diagnostics (per-route ledgers, clean route=empty slice), route_metrics (metrics::route_metrics per route — samples=the k=1 battery → convergence NA), baseline_pipeline_id, model_identity}` and passes `Some` to `assemble_report` (currently `None`); land report_ja.md beside report_en.md; `experiment_metrics` stays in-assembly (never called from run.rs). FACTS: `ModelRunSections<'a>` (report.rs:551) = `{route_diagnostics:&[(Id,&[DiagnosticRecord])], route_metrics:Vec<RouteMetrics>, baseline_pipeline_id:&Id, model_identity:&ModelIdentity}`; `assemble_report` 8th param `model_run:Option<ModelRunSections>` (report.rs:601, passed at the `assemble_report` call ~run.rs:2044). SOURCES: `RouteRun{pipeline_id,ledger,fills,groups,samples}` collected in `execute_routes` (run.rs, currently `let _ = &route_runs` ~568); `route_metrics(pipeline_id,fills,groups,samples,reference)->RouteMetrics` (metrics.rs); `baseline_pipeline_id`=`baseline_resolved.pipeline_id` (run.rs:589 `baseline_resolved`); `model_identity`=agreed (in scope, Option → `expect` on a model route); `route_diagnostics`=RouteRun.ledger's DiagnosticRecords keyed by pipeline_id; `reference`=`parse_reference(read root.join(exp.expected_outcomes))` (load `exp`+`parse_reference` LOCALLY in report_processing_stage — B2a threads `agreed`+`model_routes` but NOT `exp`/reference; those reads live INSIDE `manifest_inputs`, unreachable here — the duplicate model-route-only YAML parse is trivial); report_en landing = run.rs:2064 `write_under("report_en.md", render_markdown(&report.payload)…)` with write+read-back byte-check → add `render_markdown_ja` (report.rs:889) → `write_under("report_ja.md")` same discipline. EDITS: `report_processing_stage` gains route_runs + a LOCAL `exp`/`parse_reference` + reuses B2a's `agreed` param (now on report_processing_stage), builds sections, passes `Some` at the `assemble_report` call, lands report_ja; both tail call sites update (run.rs:246 M1 `model_run=None` / run.rs:598 M2 `Some`). Read: report.rs `assemble_report` M2 population body (below :700 — failure_taxonomy/metrics/model_identity fill) + populated_report fixture helpers (`m2_route_metrics()`/`baseline_model_identity()` for expected values) + `render_markdown_ja` tests (report.rs:3552/3697 for JA discipline); metrics.rs `RouteMetrics` shape; run.rs `report_processing_stage` body (2001-2110) + `execute_routes` RouteRun/registry-load region. Gate: fmt/clippy/doc/cargo test; `write_m2_root` pins report.json M2 sections (failure_taxonomy/metrics/model_identity) + report_en.md + report_ja.md bodies from observed; M1 report pins hold (M1 path `model_run=None` → byte-identical).
